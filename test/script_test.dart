  ...
  group('Script', () {
    ...
    group('FunctionScript', () {
      ...
      test('forwards result', () {
        return new FunctionScript(() => 0)
            .execute([]).then((result) => expect(result, 0));
      });

      test('forwards future result', () {
        return new FunctionScript(() => new Future.value(0))
            .execute([]).then((result) => expect(result, 0));
      });
      ...
      test('with sub-commands', () {
        return new FunctionScript(withSubCommands)
            .execute(['--flag1', 'recursive2', '--flag2']);
      });
    });
    ...
    group('parser', () {
      ...
      test('for Option - with allowMultiple', () {
        var optionValues;
        return new FunctionScript((
            {@Option(parser: int.parse, allowMultiple: true)
                List<int> option}) {
          optionValues = option;
      ...
      test('for Positional - from type annotation', () {
        var positionalValue;
        var restValue;
        return new FunctionScript((int first, List<int> rest) {
          positionalValue = first;
          restValue = rest;
        }).execute(['123', '4', '5', '6']).then((_) {
          expect(positionalValue, 123);
    ...
    group('ClassScript', () {
      ...
      test('args resolved', () {
        return unit
            .execute(['--flag', '--option', 'value', 'command']).then((result) {
          expect(CommandScriptTest._commandHappened, isTrue);
      ...
      test('bad base args', () {
        return unit.execute(
            ['--bogusflag', '--bogusoption', 'value', 'command']).then((_) {
      ...
      group('allowTrailingOptions', () {
        ...
        test('should not allow trailing options when false', () {
          return unit.execute([
            ...
            expect(
                _lastSeenRest, ['positional', '--command-flag', '--opt', '5']);
...
class CommandScriptTest {
  ...
  SubCommandScriptTest recursive1({bool flag1}) =>
      new SubCommandScriptTest(flag1: flag1);
}
