...
String getMarker(String command, String type) =>
...
installScript(String command, String completionSyntax) {
  scriptInstallation('install', command, completionSyntax,
...
uninstallScript(String command, String completionSyntax) {
  scriptInstallation('uninstall', command, completionSyntax, '',
      (command, rcText) {
    var parts = rcText.split(getBeginMarker(command));
    return parts.length < 2;
  }, (command, completionSyntax, rcText) {
...
String currentShell = new RegExp(r'\/bin\/(\w+)')
    .firstMatch(Platform.environment['SHELL'])
    .group(1);
File shellRc = () {
...
scriptInstallation(
    String name,
    String command,
    String completionSyntax,
    String postActionMessage,
    bool alreadyDone(String command, String rcText),
    bool action(String command, String completionSyntax, String rcText)) {
