...
Future<Iterable> getUsageCompletions(Usage usage, CommandLine commandLine) =>
    new Future.sync(() {
      ...
      if (toComplete.startsWith('-')) {
        if (commandLine.word.startsWith('--')) {
          ...
          String findByAbbr(String abbr) {
            return completionOptions.keys.firstWhere(
                (option) => completionOptions[option].abbr == abbr,
                orElse: () => null);
          }
          ...
          var allFlags =
              options.every((option) => completionOptions[option] is Flag);
        ...
        if (commandLine.partialWords.length >= 2) {
          var previousWord = commandLine.partialWords
              .elementAt(commandLine.partialWords.length - 2);
          if (previousWord.startsWith('-')) {
            ...
            if (previousWord.startsWith('--')) {
              ...
              if (previousWord.length == 2) {
                ...
                var option = completionOptions.values
                    .singleWhere((option) => option.abbr == abbr);
                return _getCompletionsForOption(option, toComplete);
              }
            }
          }
        ...
        var commandCompletions = leafUsage.commands.keys
            .where((command) => command.startsWith(toComplete));
        if (commandCompletions.isNotEmpty) return commandCompletions;

        // Try completing positional.
        var positional =
            leafUsage.positionalAt(leafCommandInvocation.positionals.length);
        if (positional != null) {
...
Future<Iterable<String>> _getCompletionsForOption(
        Option option, String prefix) =>
    new Future.sync(() {
...
Future<Iterable<String>> _getCompletionsForAllowed(allowed, String prefix) {
  ...
  return new Future<Iterable>.sync(() {
    if (allowed is Iterable)
      return allowed;
    else if (allowed is _Complete)
      return allowed();
    else if (allowed is Map) return allowed.keys;
    return [];
  }).then((Iterable completions) {
    return completions
        .map((o) => o.toString())
