...
class TerminalUsageFormatter extends UsageFormatter {
  ...
  String format() {
    ...
    if (usage.examples.isNotEmpty) {
      blocks.add([
        'Examples',
        formatColumns(usage.examples.map((ArgExample example) {
    ...
    var commandPlaceholder =
        '${commandPen('<command>')} ${textPen('[<args>]')}';
    ...
    if (visibleCommands.isNotEmpty) {
      blocks.add([
        'Commands',
        '''
${formatColumns(visibleCommands.keys.map((command) => [
                  command,
                  ((s) => s == null ? '' : s)(
                      getHelp(visibleCommands[command].description))
                ]), [commandPen, textPen])}
...
${indentLines(formatColumns(positionalsWithRest.map((positional) => [
                '<${positional.valueHelp}>',
                getHelp(positional.help)
    ...
    if (description.isNotEmpty)
      blocks.insert(0, ['Description', textPen(description)]);

    var blockStrings =
        blocks.map((block) => _formatBlock(block[0], block[1])).toList();
  ...
  String _formatCommands() => namePen(_getCommandString());
...
indentLines(String text) => const LineSplitter()
    .convert(text)
    .map((line) => indentLine(line, 2))
    .join('\n');

indentLine(String line, int spaces, {int depth: 1}) =>
    '${' ' * (spaces * depth)}$line';
